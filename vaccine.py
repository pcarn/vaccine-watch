...
def check_for_appointments():
    ...
    for location in unavailable_locations:
        # Walgreens tends to toggle their locations to unavailable, then back again, which leads to a lot of noise.
        # Don't treat Walgreens as unavailable until it has been for one hour
        if "walgreens" in location["id"]:
