...
def check_for_appointments():
    ...
    for location in unavailable_locations:
        # Walgreens tends to toggle their locations to unavailable, then back.
        # Don't treat Walgreens as unavailable until it has been for 9.5 minutes
        if "walgreens" in location["id"]:
            if redis_client.get(cache_key(location)):
                ...
                if first_unavailable_time:
                    if int(time.time()) - int(first_unavailable_time) > 570:
